/* HermesWork v3.3 — all 5 new features wired */
...
function setStatus(online, label) {
  ...
  const sub = document.querySelector('.status-sub'); if (sub) sub.textContent = online ? `${API_BASE.replace('https://','')} · ${state.health.stripe || 'stripe'} mode` : 'Check backend URL in Settings';
  ...
  setText('slack-state', state.health.slack === 'configured' ? 'Slack alerts on' : 'Slack not set');
...
async function loadAllData(silent = false) {
  try {
    ...
    setStatus(true, `Online · v${health.version || '2.4.0'}`);
    render();
    if (!silent) toast('Live data synced.');
  } catch(e) { console.warn('[HW] Load failed:', e.message); setStatus(false); render(); if (!silent) toast('Sync failed: ' + e.message, 'error'); }
}
function startRealtime() { if (state.realtimeTimer) clearInterval(state.realtimeTimer); state.realtimeTimer = setInterval(() => loadAllData(true), 15000); }
function connectSSE() {
  try {
    if (state.sse) { state.sse.close(); state.sse = null; }
    const es = new EventSource(API_BASE + '/api/stream');
    ['invoice:created','invoice:paid','invoice:updated','invoice:deleted','proposal:updated','client:created','credential:verified','update'].forEach(ev => es.addEventListener(ev, () => loadAllData(true)));
    es.onerror = () => { es.close(); state.sse = null; }; state.sse = es;
  } catch(e) {}
}
function navigate(page) {
  ...
  $(`page-${page}`)?.classList.add('active'); $(`nav-${page}`)?.classList.add('active'); $(`mob-nav-${page}`)?.classList.add('active');
...
function render() {
  ...
  setText('kpi-forecast', money(k.forecastNext || 0));
  ...
  // profile link
  const pl = $('profile-link'); if (pl) pl.href = API_BASE + '/profile/' + (state.health.profileHandle || 'salman');
...
function realEmpty(title, desc, action = '') { return `<div class="empty-state"><div class="empty-state-title">${esc(title)}</div><div class="empty-state-desc">${esc(desc)}</div>${action}</div>`; }
function renderActivity() {
  const tasks = $('scheduled-tasks-mini'); if (tasks) tasks.innerHTML = state.scheduledTasks.length ? state.scheduledTasks.slice(0,5).map(t => `<div class="cron-item"><div class="cron-status"></div><div><div class="cron-name">${esc(t.name)}</div><div class="cron-schedule">${esc(t.schedule)}</div></div><div class="cron-last-run">${esc(t.lastRun)}</div></div>`).join('') : realEmpty('No workflows loaded','Backend live.');
  const feed = $('activity-feed'); if (feed) feed.innerHTML = state.activities.length ? state.activities.slice(0,8).map(a => `<div class="activity-item"><div class="activity-dot ${esc(a.type||'invoice')}"></div><div class="activity-content"><div class="activity-text">${esc(a.action)}</div><div class="activity-time">${esc(a.time||'')}</div></div></div>`).join('') : realEmpty('No real activity yet','Create an invoice or receive a Stripe test webhook.');
}
function badge(status) { const map = { paid:'badge-paid', pending:'badge-pending', overdue:'badge-overdue', draft:'badge-draft', won:'badge-won', lost:'badge-lost' }; return `<span class="badge ${map[status]||'badge-draft'}">${esc(status||'unknown')}</span>`; }
...
function renderInvoices() {
  ...
  if (!rows.length) { tbody.innerHTML = `<tr><td colspan="7">${realEmpty('No invoices yet','Create your first real invoice.','<button class="btn btn-primary" onclick="openInvoiceModal()">Create invoice</button>')}</td></tr>`; }
  else tbody.innerHTML = rows.map(i => `<tr class="clickable-row" onclick="openDrawerById('${esc(i.id)}')">
    <td class="mono">${esc(i.id)}</td><td><strong>${esc(i.client)}</strong><br><span style="font-size:11px;color:var(--text-muted)">${esc(i.description||'')}</span></td>
    <td>${money(i.amount)}</td><td>${badge(i.status)}</td><td>${dateFmt(i.dueDate)}</td><td>${esc(i.paymentMethod||'stripe')}</td>
    <td class="table-actions" onclick="event.stopPropagation()">
      ...
      <button class="btn btn-ghost btn-xs" onclick="openPdf('${esc(i.id)}')">PDF</button>
      ${i.stripeUrl?`<button class="btn btn-ghost btn-xs" onclick="window.open('${esc(i.stripeUrl)}','_blank')">Stripe ↗</button>`:''}
...
function openPdf(id) { window.open(API_BASE + '/invoice/' + id + '/pdf', '_blank', 'noopener,noreferrer'); }

function renderClients() {
  const grid = $('clients-grid'); if (!grid) return;
  grid.innerHTML = state.clients.length ? state.clients.map(c => `<div class="client-card">
    ...
    <div class="stats-row" style="grid-template-columns:1fr 1fr;margin:0"><div class="stat-box"><div class="stat-box-value">${money(c.totalBilled)}</div><div class="stat-box-label">Billed</div></div><div class="stat-box"><div class="stat-box-value">${esc(c.paymentSpeed||'N/A')}</div><div class="stat-box-label">Speed</div></div></div>
    <div style="margin-top:12px;display:flex;gap:8px"><button class="btn btn-ghost btn-xs" onclick="createInvoiceForClient('${esc(c.id||c.name)}')">Invoice</button><button class="btn btn-ghost btn-xs" onclick="createProposalForClient('${esc(c.id||c.name)}')">Proposal</button></div>
  </div>`).join('') : `<div class="card" style="grid-column:1/-1">${realEmpty('No clients yet','Clients appear from real invoices or the Add Client button.','<button class="btn btn-primary" onclick="openClientModal()">Add client</button>')}</div>`;
}
function renderProposals() {
  ...
  tbody.innerHTML = state.proposals.length ? state.proposals.map(p => `<tr><td>${esc(p.title)}</td><td>${esc(p.client)}</td><td>${esc(p.platform)}</td><td>${money(p.amount)}</td><td>${badge(p.status)}</td><td>${dateFmt(p.sentDate)}</td><td><span class="score-pill ${(p.score||0)>=8?'high':'mid'}">${p.score||0}/10</span></td><td class="table-actions">${p.status==='pending'?`<button class="btn btn-ghost btn-xs" onclick="updateProposal('${esc(p.id)}','won')">Won</button><button class="btn btn-ghost btn-xs" style="color:#e11d48" onclick="updateProposal('${esc(p.id)}','lost')">Lost</button>`:''}</td></tr>`).join('') : `<tr><td colspan="8">${realEmpty('No proposals yet','Track your first proposal.','<button class="btn btn-primary" onclick="openProposalModal()">Add proposal</button>')}</td></tr>`;
}
function renderReputation() {
  const grid = $('reputation-grid'); if (!grid) return;
  grid.innerHTML = state.reputation.length ? state.reputation.map(r => `<div class="reputation-card">
    <div style="display:flex;justify-content:space-between;margin-bottom:14px"><strong>${esc(r.jobType)}</strong>${r.clientVerified?badge('paid'):badge('pending')}</div>
    ...
    ${r.txHash?`<div class="mono" style="cursor:pointer;font-size:11px" onclick="copyText('${esc(r.txHash)}')" title="Click to copy">${esc(hashStr(r.txHash))}</div>`:''}
    <div style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap">
      ${r.clientVerified?'<span style="font-size:11px;color:#16A34A;font-weight:600">✓ Client verified</span>':r.verifyUrl?`<button class="btn btn-ghost btn-xs" onclick="copyVerifyLink('${esc(r.verifyUrl)}')">📋 Copy verify link</button>`:''}
      ${r.minted?'<span style="font-size:11px;color:#5046e4">⛓ On-chain</span>':''}
    </div>
  </div>`).join('') : `<div class="card" style="grid-column:1/-1">${realEmpty('No payment-backed records yet','Records are created after real payment confirmation.')}</div>`;
}
function copyVerifyLink(url) { navigator.clipboard?.writeText(url).then(()=>toast('Verification link copied! Send to client.')).catch(()=>toast('Copy failed','error')); }

function renderPayments() {
  const tbody = $('payments-tbody'); if (!tbody) return;
  tbody.innerHTML = state.payments.length ? state.payments.map(p => `<tr><td>${dateFmt(p.date)}</td><td>${esc(p.client)}</td><td>${money(p.amount)}</td><td>${esc(p.rail||'stripe')}</td><td class="mono" style="cursor:pointer" onclick="copyText('${esc(p.txHash||p.stripeId||'')}')" title="Click to copy">${esc(hashStr(p.txHash||p.stripeId))}</td></tr>`).join('') : `<tr><td colspan="5">${realEmpty('No payments yet','Stripe test payments will appear here after webhook confirmation.')}</td></tr>`;
...
function renderAnalytics() {
  ...
  const forecast = state.kpis.forecastNext || state.analytics.forecastNext || 0;
  const pipeline = state.kpis.pipelineValue || state.analytics.pipelineValue || 0;
  stats.innerHTML = `<div class="stat-box"><div class="stat-box-value">${money(state.kpis.totalRevenue)}</div><div class="stat-box-label">Total revenue</div></div><div class="stat-box"><div class="stat-box-value">${state.kpis.mrrGrowth||0}%</div><div class="stat-box-label">Growth</div></div><div class="stat-box"><div class="stat-box-value" style="color:#5046e4">${money(forecast)}</div><div class="stat-box-label">Next mo. forecast</div></div><div class="stat-box"><div class="stat-box-value">${money(pipeline)}</div><div class="stat-box-label">Pipeline value</div></div>`;
  const tbody = $('hypothesis-tbody');
  if (tbody) tbody.innerHTML = (state.analytics.hypotheses||[]).map(h => `<tr><td>${esc(h.metric)}</td><td>${h.prefix||''}${esc(String(h.baseline||''))}${h.unit||''}</td><td>${h.prefix||''}${esc(String(h.target||''))}${h.unit||''}</td><td><strong>${h.prefix||''}${esc(String(h.current||'0'))}${h.unit||''}</strong></td><td>${badge(h.hit?'paid':'pending')}</td></tr>`).join('') || `<tr><td colspan="5" style="text-align:center;color:var(--text-muted);padding:20px">No hypothesis data yet.</td></tr>`;
}
function renderSettings() {
  const key = $('hermes-api-key'); if (key && !key.value) key.value = getApiKey();
  const url = $('backend-url'); if (url) url.value = API_BASE;
  const cron = $('cron-list'); if (cron) cron.innerHTML = state.scheduledTasks.length ? state.scheduledTasks.map(t => `<div class="cron-item"><div class="cron-status"></div><div><div class="cron-name">${esc(t.name)}</div><div class="cron-schedule">${esc(t.schedule)}</div></div><div class="cron-last-run">${esc(t.lastRun||'pending')}</div></div>`).join('') : `<div style="padding:16px;color:var(--text-muted);font-size:13px">No scheduled tasks yet.</div>`;
  const healthEl = $('backend-health');
  if (healthEl && state.health.version) healthEl.innerHTML = `<div class="health-row"><span>Version</span><strong>${esc(state.health.version||'—')}</strong></div><div class="health-row"><span>Stripe</span><strong>${esc(state.health.stripe||'—')}</strong></div><div class="health-row"><span>ERC-8004</span><strong>${esc(state.health.erc8004||'—')}</strong></div><div class="health-row"><span>Slack</span><strong>${esc(state.health.slack||'—')}</strong></div><div class="health-row"><span>API key</span><strong>${esc(state.health.apiKey||'—')}</strong></div><div class="health-row"><span>Invoices</span><strong>${state.health.data?.invoices??'—'}</strong></div><div class="health-row"><span>Uptime</span><strong>${state.health.uptime||'—'}s</strong></div><div class="health-row"><span>Profile URL</span><a href="${API_BASE}/profile/${state.health.profileHandle||'salman'}" target="_blank" style="color:#5046e4;font-size:12px">/profile/${esc(state.health.profileHandle||'salman')} ↗</a></div>`;
}
function drawCharts() {
  ...
  // Forecast overlay on revenue chart
  const forecast = state.kpis.forecastNext || 0;
  if (charts['chart-revenue'] && forecast > 0) {
    charts['chart-revenue'].data.labels = [...labels, 'Forecast'];
    charts['chart-revenue'].data.datasets = [{ data:[...revenueData, 0], backgroundColor:'#5046e4', borderRadius:6, borderSkipped:false, label:'Actual' },{ data:[...Array(6).fill(null), forecast], backgroundColor:'rgba(80,70,228,0.35)', borderRadius:6, borderDash:[4,4], label:'Forecast' }];
    charts['chart-revenue'].options.plugins.legend.display = true;
    charts['chart-revenue'].update();
  }
  const won=state.proposals.filter(p=>p.status==='won').length; const lost=state.proposals.filter(p=>p.status==='lost').length; const pending=state.proposals.filter(p=>p.status==='pending').length;
  mkChart('chart-winrate','doughnut',{ labels:['Won','Lost','Pending'], datasets:[{ data:[won||0,lost||0,pending||0], backgroundColor:['#16A34A','#e11d48','#f59e0b'], borderWidth:0 }] },{ plugins:{ legend:{ display:true, position:'bottom' } }, scales:{} });
  const daysData = state.analytics.daysToPayment || Array(6).fill(0);
  mkChart('chart-days','line',{ labels:labels.slice(0,daysData.length), datasets:[{ data:daysData, borderColor:'#5046e4', backgroundColor:'rgba(80,70,228,0.08)', fill:true, tension:0.4, pointRadius:4 }] });
  const repData = state.analytics.credentialsPerMonth || Array(6).fill(0);
  mkChart('chart-credentials','bar',{ labels:labels.slice(0,repData.length), datasets:[{ data:repData, backgroundColor:'#16A34A', borderRadius:6 }] });
}
...
async function submitInvoice(e) {
  e.preventDefault(); const btn=e.target.querySelector('[type=submit]'); btn.disabled=true; btn.textContent='Creating…';
  try {
    ...
    toast('Invoice created!'); closeInvoiceModal(); $('invoice-form').reset(); await loadAllData(true);
...
function openDrawerById(id) {
  ...
  const title=$('drawer-title'); const body=$('drawer-body'); const drawer=$('invoice-drawer'); const backdrop=$('drawer-backdrop');
  if (!drawer||!body) return;
  if (title) title.textContent=`Invoice ${inv.id}`;
  body.innerHTML=`<div class="drawer-field"><span class="drawer-label">Client</span><span class="drawer-value">${esc(inv.client)}</span></div>
    ...
    ${inv.txHash?`<div class="drawer-field"><span class="drawer-label">Tx hash</span><span class="drawer-value mono" style="cursor:pointer" onclick="copyText('${esc(inv.txHash)}')">${esc(hashStr(inv.txHash))}</span></div>`:''}
    <div class="drawer-actions">
      <button class="btn btn-primary" onclick="copyPayLink('${esc(inv.id)}')">Copy payment link</button>
      <button class="btn btn-secondary" onclick="openPdf('${esc(inv.id)}')">⬇ PDF</button>
...
/* ─── Proposal actions ─── */
function openProposalModal(clientName='') { const m=$('proposal-modal'); if (m) { m.classList.add('open'); if (clientName) { const c=$('prop-client'); if(c) c.value=clientName; } $('prop-title')?.focus(); } }
...
async function submitProposal(e) {
  ...
  try { const body={title:$('prop-title').value,client:$('prop-client').value,platform:$('prop-platform').value,amount:Number($('prop-amount').value),status:$('prop-status').value,sentDate:new Date().toISOString()}; await apiFetch('/api/proposals',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}); toast('Proposal added!'); closeProposalModal(); $('proposal-form').reset(); await loadAllData(true); }
  catch(err) { toast('Error: '+err.message,'error'); } finally { btn.disabled=false; btn.textContent='Add proposal'; }
}
async function updateProposal(id, status) { try { await apiFetch(`/api/proposals/${id}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({status})}); toast(`Proposal marked ${status}!`); await loadAllData(true); } catch(e) { toast(e.message,'error'); } }
...
async function submitClient(e) {
  e.preventDefault(); const btn=e.target.querySelector('[type=submit]'); btn.disabled=true; btn.textContent='Saving…';
  try { const body={name:$('cli-name').value,company:$('cli-company').value,industry:$('cli-industry').value,email:$('cli-email').value}; await apiFetch('/api/clients',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}); toast('Client added!'); closeClientModal(); $('client-form').reset(); await loadAllData(true); }
  catch(err) { toast('Error: '+err.message,'error'); } finally { btn.disabled=false; btn.textContent='Add client'; }
}
function createInvoiceForClient(id) { openInvoiceModal(); setTimeout(()=>{ const c=$('inv-client'); if(c) c.value=id; },50); }
...
function testBackend() { const url=$('backend-url')?.value?.trim(); if(url){ localStorage.setItem('HERMESWORK_BACKEND_URL',url); } loadAllData(false); }
...
function buildCmdItems() {
  cmdItems = [
    ...
    { label:'View public profile', icon:'🛡', action:()=>{ closeCmdPalette(); window.open(API_BASE+'/profile/'+(state.health.profileHandle||'salman'),'_blank'); } },
...
function renderCmdResults(q) {
  ...
  list.innerHTML=filtered.slice(0,12).map((item,i)=>`<div class="cmd-item${i===cmdSelected?' selected':''}" onclick="cmdItems.find(x=>x.label===unescape('${escape(item.label)}'))?.action()"><span class="cmd-icon">${item.icon}</span><span>${esc(item.label)}</span></div>`).join('')||`<div class="cmd-empty">No results for "${esc(q)}"</div>`;
}
function handleCmdKey(e) {
  const list=$('cmd-results'); if(!list) return; const items=list.querySelectorAll('.cmd-item');
...
function initKeyboard() {
  document.addEventListener('keydown', e => {
    const tag=document.activeElement?.tagName; const inInput=tag==='INPUT'||tag==='TEXTAREA'||tag==='SELECT';
    ...
    if(state._gPressed){ state._gPressed=false; const map={d:'dashboard',i:'invoices',c:'clients',p:'proposals',r:'reputation',a:'analytics',s:'settings',x:'payments'}; if(map[e.key]){ navigate(map[e.key]); return; } }
  });
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const ki=$('hermes-api-key'); if(ki) ki.value=getApiKey();
