/* HermesWork Frontend v11.1 — current backend route compatibility */
...
function setStatus(online, label) {
  ...
  setText('slack-state', `Agents ${state.health.agents || state.dashboard.liveMetrics?.agentsActive || 36}`);
...
function kpisFromDashboard(dash, health) {
  ...
  return {
    mrr: lm.totalRevenue || 0,
    totalRevenue: lm.totalRevenue || 0,
    activeInvoices,
    outstandingValue: lm.activeValue || 0,
    winRate: lm.winRate || 0,
    reputationScore: 0,
    daysToPayment: 0,
    activeProjects: activeInvoices,
    forecastNext: rev.forecastConversion || 0,
    forecastNextMonth: rev.forecastConversion || 0,
    pipelineValue: rev.pipelineValue || 0,
    monthlyRevenue: (rev.sparkline || []).map(x => Number(x.revenue || 0)),
    monthLabels: (rev.sparkline || []).map(x => x.month),
    agents: health.agents || lm.agentsActive || 36,
    mcpTools: health.mcpTools || lm.mcpTools || 60
  };
}
...
async function loadAllData(silent = false) {
  try {
    const [health, dash] = await Promise.all([
      publicFetch('/health'),
      publicFetch('/dashboard/live').catch(() => ({}))
    ]);
    state.health = health || {};
    ...
    // Public mode works without API key. If key exists, load full private records.
    if (getApiKey()) {
      ...
      state.reputation = [];
      state.payments = [];
      state.analytics = { monthLabels: state.kpis.monthLabels, monthlyRevenue: state.kpis.monthlyRevenue };
    }

    state.lastSync = new Date();
    setStatus(true, `Online · ${health.version || 'v11.0.0'} · ${health.agents || 36} agents`);
    render();
    if (!silent) toast(getApiKey() ? 'Live data synced.' : 'Public dashboard connected. Add API key for private records.');
  } catch (e) {
...
function connectSSE() {
  ...
  try {
    ...
    const es = new EventSource(API_BASE + '/events?apiKey=' + encodeURIComponent(getApiKey()));
...
function renderActivity() {
  ...
  if (tasks) {
    const swarm = state.dashboard.revenueSwarm;
    tasks.innerHTML = `
...
function renderInvoices() {
  ...
  if (!getApiKey()) tbody.innerHTML = `<tr><td colspan="7">${realEmpty('Private invoices hidden','Dashboard is connected in public mode. Paste HERMESWORK_API_KEY in Settings to load invoices.','<button class="btn btn-primary" onclick="navigate(\'settings\')">Add API key</button>')}</td></tr>`;
  else if (!rows.length) tbody.innerHTML = `<tr><td colspan="7">${realEmpty('No invoices yet','Create your first real invoice.','<button class="btn btn-primary" onclick="openInvoiceModal()">Create invoice</button>')}</td></tr>`;
...
function renderProposals() {
  ...
  if (!getApiKey()) tbody.innerHTML = `<tr><td colspan="8">${realEmpty('Private proposals hidden','Add API key in Settings to load proposal pipeline.')}</td></tr>`;
  else tbody.innerHTML = state.proposals.length ? state.proposals.map(p => `<tr><td>${esc(p.title)}</td><td>${esc(p.client)}</td><td>${esc(p.platform)}</td><td>${money(p.amount)}</td><td>${badge(p.status)}</td><td>${dateFmt(p.sentDate)}</td><td><span class="score-pill ${(p.score||0)>=8?'high':'mid'}">${p.score||0}/10</span></td><td class="table-actions">${p.status==='pending'?`<button class="btn btn-ghost btn-xs" onclick="updateProposal('${esc(p.id)}','won')">Won</button><button class="btn btn-ghost btn-xs" style="color:#e11d48" onclick="updateProposal('${esc(p.id)}','lost')">Lost</button>`:''}</td></tr>`).join('') : `<tr><td colspan="8">${realEmpty('No proposals yet','Track your first proposal.','<button class="btn btn-primary" onclick="openProposalModal()">Add proposal</button>')}</td></tr>`;
}
function renderReputation() {
  const grid = $('reputation-grid'); if (!grid) return;
  if (!getApiKey()) { grid.innerHTML = `<div class="card" style="grid-column:1/-1">${realEmpty('Private reputation records hidden','Public profile remains available. Add API key for detailed records.')}</div>`; return; }
  grid.innerHTML = state.reputation.length ? state.reputation.map(r => `<div class="reputation-card"><div style="display:flex;justify-content:space-between;margin-bottom:14px"><strong>${esc(r.jobType || r.jobCategory || 'Work')}</strong>${r.clientVerified?badge('paid'):badge('pending')}</div><div style="font-size:26px;font-weight:800;color:var(--gold)">${money(r.amount)}</div><div style="font-size:12px;color:var(--muted);margin:8px 0">${esc(r.client)} · ${dateFmt(r.date)} · ${esc(r.paymentRail||'stripe')}</div>${r.txHash?`<div class="mono" style="cursor:pointer;font-size:11px" onclick="copyText('${esc(r.txHash)}')">${esc(hashStr(r.txHash))}</div>`:''}</div>`).join('') : `<div class="card" style="grid-column:1/-1">${realEmpty('No payment-backed records yet','Records are created after real payment confirmation.')}</div>`;
}
function renderPayments() {
  const tbody = $('payments-tbody'); if (!tbody) return;
  if (!getApiKey()) tbody.innerHTML = `<tr><td colspan="5">${realEmpty('Private payments hidden','Add API key in Settings to load payment records.')}</td></tr>`;
  else tbody.innerHTML = state.payments.length ? state.payments.map(p => `<tr><td>${dateFmt(p.date)}</td><td>${esc(p.client)}</td><td>${money(p.amount)}</td><td>${esc(p.rail||'stripe')}</td><td class="mono" style="cursor:pointer" onclick="copyText('${esc(p.txHash||p.stripeId||'')}')">${esc(hashStr(p.txHash||p.stripeId))}</td></tr>`).join('') : `<tr><td colspan="5">${realEmpty('No payments yet','Stripe test payments will appear here after webhook confirmation.')}</td></tr>`;
...
function renderAnalytics() {
  ...
  const tbody = $('hypothesis-tbody'); if (tbody) tbody.innerHTML = `<tr><td>Revenue Swarm</td><td>v10 manual</td><td>autonomous loop</td><td><strong>v11 active</strong></td><td>${badge('paid')}</td></tr><tr><td>Agents</td><td>31</td><td>36</td><td><strong>${state.health.agents || 36}</strong></td><td>${badge('paid')}</td></tr>`;
}
...
function requireKey() { if (!getApiKey()) { toast('Add HERMESWORK_API_KEY in Settings first.', 'error'); navigate('settings'); return false; } return true; }
function openInvoiceModal() { if (!requireKey()) return; const m=$('invoice-modal'); if (m) { m.classList.add('open'); $('inv-client')?.focus(); } }
function closeInvoiceModal() { $('invoice-modal')?.classList.remove('open'); }
async function submitInvoice(e) {
  e.preventDefault(); if (!requireKey()) return;
...
async function markPaid(id) { if (!requireKey() || !confirm(`Mark invoice ${id} as paid?`)) return; try { await apiFetch(`/invoices/${id}/pay`, { method:'PATCH' }); toast('Marked paid!'); await loadAllData(true); } catch(e) { toast(e.message,'error'); } }
async function deleteInvoice(id) { if (!requireKey() || !confirm(`Delete invoice ${id}?`)) return; try { await apiFetch(`/invoices/${id}`, { method:'DELETE' }); toast('Invoice deleted.'); closeDrawer(); await loadAllData(true); } catch(e) { toast(e.message,'error'); } }
...
function openProposalModal(clientName='') { if (!requireKey()) return; const m=$('proposal-modal'); if (m) { m.classList.add('open'); if (clientName && $('prop-client')) $('prop-client').value=clientName; $('prop-title')?.focus(); } }
function closeProposalModal() { $('proposal-modal')?.classList.remove('open'); }
async function submitProposal(e) { e.preventDefault(); if (!requireKey()) return; const btn=e.target.querySelector('[type=submit]'); if(btn){btn.disabled=true;btn.textContent='Saving…';} try { const body={title:$('prop-title').value,client:$('prop-client').value,platform:$('prop-platform').value,amount:Number($('prop-amount').value),status:$('prop-status').value}; await apiFetch('/proposals',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}); toast('Proposal added!'); closeProposalModal(); $('proposal-form')?.reset(); await loadAllData(true); } catch(err) { toast('Error: '+err.message,'error'); } finally { if(btn){btn.disabled=false;btn.textContent='Add proposal';} } }
async function updateProposal(id, status) { if (!requireKey()) return; try { await apiFetch(`/proposals/${id}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({status})}); toast(`Proposal marked ${status}!`); await loadAllData(true); } catch(e) { toast(e.message,'error'); } }

function openClientModal() { if (!requireKey()) return; $('client-modal')?.classList.add('open'); $('cli-name')?.focus(); }
function closeClientModal() { $('client-modal')?.classList.remove('open'); }
async function submitClient(e) { e.preventDefault(); if (!requireKey()) return; const btn=e.target.querySelector('[type=submit]'); if(btn){btn.disabled=true;btn.textContent='Saving…';} try { const body={name:$('cli-name').value,company:$('cli-company').value,industry:$('cli-industry').value,email:$('cli-email').value}; await apiFetch('/clients',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)}); toast('Client added!'); closeClientModal(); $('client-form')?.reset(); await loadAllData(true); } catch(err) { toast('Error: '+err.message,'error'); } finally { if(btn){btn.disabled=false;btn.textContent='Add client';} } }
...
function saveApiKeyFromField() { const k=$('hermes-api-key')?.value?.trim(); if(k){ localStorage.setItem('HERMESWORK_API_KEY',k); toast('API key saved. Syncing…'); loadAllData(false); connectSSE(); } else toast('Enter a key first','error'); }
...
function buildCmdItems() {
  cmdItems = [
    ...
    { label:'Open v11 agents', icon:'🧪', action:()=>{ closeCmdPalette(); window.open(API_BASE+'/v11/agents','_blank'); } },
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if ($('hermes-api-key')) $('hermes-api-key').value=getApiKey();
