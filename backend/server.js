...
// ============================================================
// MCP TOOLS LIST (21 tools — 15 ops + 6 AI-powered)
// ============================================================
const MCP_TOOLS = [
  // ── Operations (15) ──
  ...
  { name: 'generate_proposal', description: '✨ AI: Generate a compelling freelance proposal using Hermes 3. Pass job details and get a ready-to-send proposal.', inputSchema: { type:'object', properties: { jobTitle:{type:'string',description:'Job/project title'}, client:{type:'string',description:'Client or company name'}, budget:{type:'number',description:'Budget in USD'}, requirements:{type:'string',description:'Job requirements or brief description'}, mySkills:{type:'string',description:'Your relevant skills (optional)'} }, required:['jobTitle','client','requirements'] } },
  ...
  { name: 'run_daily_operations', description: '✨ AI AUTONOMOUS: Run full daily operations. Hermes 3 checks overdue invoices, scores proposals, calculates what to prioritize, and returns an action plan with next steps ready to execute.', inputSchema: { type:'object', properties: { autoRemind:{type:'boolean',description:'Automatically send reminders for overdue invoices (default: false, ask first)'} } } }
];
...
async function executeMcpTool(toolName, args, apiKeyOk) {
  ...
  // ──── AI-POWERED TOOLS (Hermes 3 via NVIDIA NIM) ────
  ...
  if (toolName==='generate_proposal') {
    ...
    const system = `You are a top-tier freelance business strategist. Write compelling, specific, and concise freelance proposals that win contracts. Focus on client value, not self-promotion. Be direct and professional. Max 300 words.`;
    ...
    logActivity(`[AI] Proposal generated for ${client}: ${jobTitle}`, 'ai');
    return { proposal, jobTitle, client, budget, model: AI_MODEL, provider: NVIDIA_NIM_API_KEY ? 'NVIDIA NIM' : 'Nous Portal', wordCount: proposal.split(' ').length };
  }
  ...
  if (toolName==='suggest_rate') {
    ...
    const system = `You are a freelance pricing expert. Give specific, data-backed rate recommendations. Be direct with numbers. Max 200 words.`;
    const user = `Suggest optimal rate for this project:\n\nProject type: ${projectType}\nEstimated hours: ${hoursEstimate || 'unknown'}\nClient budget: ${clientBudget ? '$'+clientBudget : 'not stated'}\n\nMy stats: ${kpis.winRate}% win rate, avg winning bid $${avgWon}, ${kpis.credentialsMinted} verified credentials, ${kpis.reputationLevel} level.\n\nGive: 1) Recommended hourly rate 2) Recommended project rate 3) Floor price (don't go below) 4) Negotiation strategy`;
    const advice = await callHermes(system, user, 400);
    logActivity(`[AI] Rate advice for: ${projectType}`, 'ai');
    return { projectType, advice, myStats: { winRate:kpis.winRate, avgWinningBid:avgWon, reputationLevel:kpis.reputationLevel }, model: AI_MODEL, provider: NVIDIA_NIM_API_KEY ? 'NVIDIA NIM' : 'Nous Portal' };
  }
...
app.get('/mcp/manifest', (req, res) => {
  res.json({ schemaVersion:'1.0', name:'hermeswork', displayName:'HermesWork — AI Freelance Operations', description:'Gives Hermes Agent full control of a freelance business with 21 tools: create Stripe invoices, track payments, manage clients, write AI proposals with Hermes 3, get autonomous daily briefings, and run daily operations hands-free.', version:'3.0.0', author:'HermesWork', icon:'🦊', server:{url:PUBLIC_BASE_URL+'/mcp',transport:'http',method:'POST'}, authentication:{type:'apiKey',header:'x-api-key'}, aiPowered:{provider:NVIDIA_NIM_API_KEY?'NVIDIA NIM':NOUS_API_KEY?'Nous Portal':'not_configured',model:AI_MODEL,tools:['generate_proposal','analyze_client','suggest_rate','draft_followup','ai_briefing','run_daily_operations']}, tools:MCP_TOOLS });
});
...
app.post('/mcp', asyncWrap(async (req, res) => {
  ...
  if(method==='initialize') return ok({protocolVersion:'2024-11-05',serverInfo:{name:'hermeswork',version:'3.0.0',description:'AI-Powered Freelance Operations MCP Server — Hermes 3 via NVIDIA NIM'},capabilities:{tools:{}}});
...
app.get('/mcp/stream', (req,res)=>{
  ...
  res.write(`event: ready\ndata: {"server":"hermeswork","version":"3.0.0","tools":${MCP_TOOLS.length},"aiEnabled":${!!AI_API_KEY},"provider":"${NVIDIA_NIM_API_KEY?'NVIDIA NIM':NOUS_API_KEY?'Nous Portal':'none'}"}\n\n`);
...
// REST ROUTES
// ============================================================
app.get('/', (req,res)=>res.json({name:'HermesWork API',status:'ok',version:'3.0.0',ai:{enabled:!!AI_API_KEY,provider:NVIDIA_NIM_API_KEY?'NVIDIA NIM':NOUS_API_KEY?'Nous Portal':'not configured',model:AI_MODEL,tools:6},mcp:{manifest:PUBLIC_BASE_URL+'/mcp/manifest',endpoint:PUBLIC_BASE_URL+'/mcp',tools:MCP_TOOLS.length},timestamp:new Date().toISOString()}));

app.get('/health',(req,res)=>res.json({status:'ok',version:'3.0.0',env:NODE_ENV,uptime:Math.round(process.uptime()),memory:Math.round(process.memoryUsage().heapUsed/1024/1024)+'MB',data:{invoices:db.invoices.length,clients:db.clients.length,proposals:db.proposals.length,credentials:db.reputation.length},stripe:stripe?'connected':'not_configured',erc8004:(process.env.PRIVATE_KEY&&!process.env.PRIVATE_KEY.startsWith('0x_')&&process.env.ERC8004_REGISTRY)?'configured':'not_configured',slack:SLACK_WEBHOOK_URL?'configured':'not_configured',ai:{enabled:!!AI_API_KEY,provider:NVIDIA_NIM_API_KEY?'NVIDIA NIM':NOUS_API_KEY?'Nous Portal':'not_configured',model:AI_MODEL},mcp:{endpoint:'/mcp',manifest:'/mcp/manifest',tools:MCP_TOOLS.length},apiKey:API_KEY?'configured':'not_configured',profileHandle:PROFILE_HANDLE,sseClients:sseClients.size,timestamp:new Date().toISOString()}));
...
app.get('/api/activity',(req,res)=>res.json({activities:db.activities.slice(0,30),scheduledTasks:[{name:'Daily Ops (Hermes 3)',schedule:'0 9 * * *',lastRun:'Today 09:00',action:'AI analyzes business + sends overdue reminders',status:'active'},{name:'Weekly KPI Report',schedule:'0 8 * * 1',lastRun:'Mon 08:00',action:'Generates weekly summary',status:'active'},{name:'Job Board Scanner',schedule:'*/30 * * * *',lastRun:'30 min ago',action:'Scans matching jobs',status:'active'},{name:'ERC-8004 Sync',schedule:'0 0 * * *',lastRun:'Today 00:00',action:'Syncs credentials',status:'active'}],systemStatus:'active',uptime:Math.round(process.uptime()/3600)+'h '+Math.round((process.uptime()%3600)/60)+'m',aiEnabled:!!AI_API_KEY}));
...
function startServer(){
  app.listen(PORT,()=>{
    console.log('\n==========================================');
    console.log('  HermesWork Backend v3.0.0');
    ...
    console.log('  MCP:      '+PUBLIC_BASE_URL+'/mcp  ('+MCP_TOOLS.length+' tools, '+MCP_TOOLS.filter(t=>t.name.startsWith('generate')||t.name.startsWith('analyze')||t.name.startsWith('suggest')||t.name.startsWith('draft')||t.name.startsWith('ai')||t.name.startsWith('run')).length+' AI-powered)');
