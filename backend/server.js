...
async function mintERC8004(jobData) {
  ...
  } catch(e) { console.error('[ERC-8004] Mint failed:', e.message); return { skipped: true, reason: e.message }; }
}
...
// ── ROUTES ──
app.get('/', (req, res) => res.json({ name: 'HermesWork API', status: 'ok', version: '2.4.0', timestamp: new Date().toISOString() }));
...
app.patch('/api/invoices/:id', requireApiKey, asyncWrap(async (req, res) => {
  ...
  if (req.body.status && ['pending','paid','overdue','draft'].includes(req.body.status)) { invoice.status = req.body.status; if (req.body.status === 'paid' && !invoice.paidAt) { invoice.paidAt = new Date().toISOString(); await notifySlack(`💰 Invoice *${invoice.id}* marked paid — $${invoice.amount} from *${invoice.client}*`, [{type:'section',text:{type:'mrkdwn',text:`✅ *Invoice Paid*\n*Client:* ${invoice.client}\n*Amount:* $${invoice.amount}\n*Invoice:* ${invoice.id}`}}]); } }
  ...
  logActivity(`Invoice ${invoice.id} updated — ${invoice.status}`, 'invoice'); saveData();
...
app.delete('/api/invoices/:id', requireApiKey, (req, res) => {
  ...
  const [removed] = db.invoices.splice(idx, 1); logActivity(`Invoice ${removed.id} deleted — ${removed.client}`, 'invoice'); saveData(); broadcastSSE('invoice:deleted', { id: removed.id }); res.json({ success: true, deleted: removed.id });
});
...
app.get('/invoice/:id/pdf', (req, res) => {
  ...
  res.send(`<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>Invoice ${inv.id}</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:system-ui,sans-serif;color:#0f172a;background:#fff;padding:40px;max-width:680px;margin:auto}.header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:40px;padding-bottom:24px;border-bottom:2px solid #e2e8f0}.logo{font-size:24px;font-weight:800;letter-spacing:-1px}.logo span{color:#5046e4}h1{font-size:32px;font-weight:800;margin-bottom:4px}.inv-id{color:#94a3b8;font-size:14px}.status{display:inline-block;background:${statusColor};color:#fff;padding:4px 14px;border-radius:20px;font-size:12px;font-weight:700;text-transform:uppercase;margin-bottom:24px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:32px;margin:32px 0}.label{font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.06em;color:#94a3b8;margin-bottom:6px}.value{font-size:15px;font-weight:500}.amount-box{background:#f8f7ff;border:2px solid #5046e4;border-radius:12px;padding:24px;text-align:center;margin:32px 0}.amount-label{font-size:13px;color:#5046e4;font-weight:600;margin-bottom:8px}.amount-value{font-size:40px;font-weight:900;color:#5046e4}.pay-link{margin-top:12px;font-size:12px;color:#94a3b8}.desc-box{background:#f8fafc;border-radius:8px;padding:16px;margin:24px 0}.footer{margin-top:40px;padding-top:24px;border-top:1px solid #e2e8f0;text-align:center;font-size:12px;color:#94a3b8}@media print{.no-print{display:none}}</style></head><body><div class="header"><div><div class="logo">Hermes<span>Work</span></div><div style="font-size:12px;color:#94a3b8;margin-top:4px">Autonomous Freelance Operations</div></div><div style="text-align:right"><h1>${inv.id}</h1><div class="inv-id">Issued ${new Date(inv.createdAt+'T00:00:00Z').toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'})}</div></div></div><div class="status">${inv.status}</div><div class="grid"><div><div class="label">Billed To</div><div class="value" style="font-size:18px;font-weight:700">${inv.client}</div></div><div><div class="label">Payment Rail</div><div class="value">${inv.paymentMethod||'Stripe'}</div></div><div><div class="label">Due Date</div><div class="value">${new Date(inv.dueDate+'T00:00:00Z').toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'})}</div></div><div><div class="label">${inv.status==='paid'?'Paid On':'Status'}</div><div class="value">${inv.paidAt?new Date(inv.paidAt).toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'}):inv.status}</div></div></div>${inv.description?`<div class="desc-box"><div class="label" style="margin-bottom:6px">Description</div><div style="font-size:14px">${inv.description}</div></div>`:''}<div class="amount-box"><div class="amount-label">Total Amount Due</div><div class="amount-value">$${Number(inv.amount).toLocaleString('en-US',{minimumFractionDigits:2})}</div>${inv.stripeUrl?`<div class="pay-link">Pay at: ${inv.stripeUrl}</div>`:''}</div>${inv.txHash?`<div class="desc-box"><div class="label" style="margin-bottom:6px">Blockchain Record</div><div style="font-size:12px;font-family:monospace;word-break:break-all">${inv.txHash}</div></div>`:''}<div class="footer">Generated by HermesWork · ${PUBLIC_BASE_URL} · ${new Date().toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'})}</div><div class="no-print" style="margin-top:32px;text-align:center"><button onclick="window.print()" style="background:#5046e4;color:#fff;border:none;padding:12px 32px;border-radius:8px;font-size:16px;font-weight:600;cursor:pointer">⬇ Save as PDF</button></div></body></html>`);
});
...
app.post('/invoice/create', requireApiKey, validate({ client:{required:true,maxLen:100}, amount:{required:true,type:'number',min:0.01,max:1000000}, dueDate:{required:true,date:true}, paymentMethod:{enum:['stripe','x402','both']} }), asyncWrap(async (req, res) => {
  ...
  if (stripe && (paymentMethod === 'stripe' || paymentMethod === 'both')) {
    ...
    } catch(e) { console.error('[Stripe] Invoice failed:', e.message); invoice.stripeError = e.message; }
  ...
  saveData(); broadcastSSE('invoice:created', { id: invId, client, amount });
  await notifySlack(`📄 New invoice *${invId}* created — $${amount} for *${client}*`, [{type:'section',text:{type:'mrkdwn',text:`📄 *New Invoice Created*\n*Client:* ${client}\n*Amount:* $${amount}\n*Due:* ${dueDate}\n*Rail:* ${paymentMethod}`}}]);
  res.status(201).json({ success:true, invoice });
}));

app.post('/invoice/send/:id', requireApiKey, asyncWrap(async (req, res) => {
  const invoice = db.invoices.find(i => i.id === req.params.id); if (!invoice) return res.status(404).json({ error: 'Invoice not found' });
  if (stripe && invoice.stripeId) { try { await stripe.invoices.sendInvoice(invoice.stripeId); } catch(e) { console.warn('[Stripe] Re-send:', e.message); } }
...
app.post('/pay/:invoiceId/confirm', asyncWrap(async (req, res) => {
  ...
  db.reputation.unshift(cred); logActivity('x402 payment confirmed — '+invoice.id+' — $'+invoice.amount, 'blockchain');
  saveData(); broadcastSSE('invoice:paid', { id:invoice.id, amount:invoice.amount, client:invoice.client });
  await notifySlack(`⚡ x402 payment confirmed — *${invoice.id}* — $${invoice.amount} from *${invoice.client}*\nVerify: ${PUBLIC_BASE_URL}/verify/${verifyToken}`);
...
app.get('/verify/:token', (req, res) => {
  ...
  if (!cred) return res.status(404).send(`<!DOCTYPE html><html><head><title>Verification Not Found</title><style>body{font-family:system-ui,sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;background:#fef2f2}.box{background:#fff;border-radius:16px;padding:40px;text-align:center;max-width:400px;box-shadow:0 8px 32px rgba(0,0,0,.12)}.icon{font-size:48px;margin-bottom:16px}h2{color:#DC2626;margin-bottom:8px}p{color:#64748b;font-size:14px}</style></head><body><div class="box"><div class="icon">❌</div><h2>Link Not Found</h2><p>This verification link is invalid or has expired.</p></div></body></html>`);
  res.send(`<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Verify Payment — HermesWork</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:system-ui,sans-serif;background:linear-gradient(135deg,#f8f7ff 0%,#ede9fe 100%);min-height:100vh;display:flex;align-items:center;justify-content:center;padding:20px}.card{background:#fff;border-radius:20px;padding:40px;max-width:480px;width:100%;box-shadow:0 12px 40px rgba(80,70,228,.15)}.logo{font-size:20px;font-weight:800;color:#0f172a;margin-bottom:32px}.logo span{color:#5046e4}.verified-badge{display:inline-block;background:#dcfce7;color:#166534;padding:6px 16px;border-radius:20px;font-size:12px;font-weight:700;margin-bottom:20px}.pending-badge{display:inline-block;background:#fef9c3;color:#854d0e;padding:6px 16px;border-radius:20px;font-size:12px;font-weight:700;margin-bottom:20px}h2{font-size:24px;font-weight:800;margin-bottom:8px}p.sub{color:#64748b;font-size:14px;margin-bottom:28px}.info-grid{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:28px}.info-item{background:#f8fafc;border-radius:10px;padding:14px}.info-label{font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.06em;color:#94a3b8;margin-bottom:4px}.info-value{font-size:16px;font-weight:700;color:#0f172a}form{display:flex;flex-direction:column;gap:14px}input,textarea{border:1.5px solid #e2e8f0;border-radius:8px;padding:12px;font-size:15px;width:100%;outline:none;transition:border-color .15s}input:focus,textarea:focus{border-color:#5046e4}textarea{resize:none;height:80px}button{background:#5046e4;color:#fff;border:none;border-radius:10px;padding:14px;font-size:16px;font-weight:700;cursor:pointer;transition:background .15s}button:hover{background:#4338ca}button:disabled{background:#94a3b8;cursor:not-allowed}.success{background:#f0fdf4;border:1.5px solid #86efac;border-radius:12px;padding:20px;text-align:center;display:none}.success h3{color:#166534;margin-bottom:6px}.success p{color:#15803d;font-size:14px}</style></head><body><div class="card"><div class="logo">Hermes<span>Work</span></div>${cred.clientVerified?'<div class="verified-badge">✓ Already Verified</div>':'<div class="pending-badge">⏳ Awaiting Verification</div>'}<h2>${cred.clientVerified?'Payment Verified':'Confirm Your Payment'}</h2><p class="sub">${cred.clientVerified?'This payment has been client-verified and recorded on the blockchain-backed reputation system.':'Please confirm you received the work and made this payment. This helps build a verified work record.'}</p><div class="info-grid"><div class="info-item"><div class="info-label">Job Type</div><div class="info-value">${cred.jobType}</div></div><div class="info-item"><div class="info-label">Amount Paid</div><div class="info-value">$${Number(cred.amount).toLocaleString()}</div></div><div class="info-item"><div class="info-label">Date</div><div class="info-value">${cred.date}</div></div><div class="info-item"><div class="info-label">Payment Rail</div><div class="info-value">${cred.paymentRail||'Stripe'}</div></div></div>${!cred.clientVerified?`<div class="success" id="success-msg"><h3>✅ Verified!</h3><p>Thank you. This record has been marked as client-verified.</p></div><form onsubmit="submitVerify(event)"><input type="text" id="verify-name" placeholder="Your name (optional)" maxlength="100"><textarea id="verify-note" placeholder="Optional note about the work…" maxlength="300"></textarea><button type="submit" id="verify-btn">Confirm I Made This Payment</button></form>`:''}</div><script>async function submitVerify(e){e.preventDefault();const btn=document.getElementById('verify-btn');btn.disabled=true;btn.textContent='Verifying…';try{const res=await fetch(window.location.href,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:document.getElementById('verify-name').value,note:document.getElementById('verify-note').value})});if(res.ok){document.getElementById('success-msg').style.display='block';document.querySelector('form').style.display='none';}else{btn.disabled=false;btn.textContent='Confirm I Made This Payment';alert('Verification failed. Try again.');}}catch(err){btn.disabled=false;btn.textContent='Confirm I Made This Payment';alert('Error: '+err.message);}}</script></body></html>`);
});
app.post('/verify/:token', asyncWrap(async (req, res) => {
  const cred = db.reputation.find(r => r.verifyToken === req.params.token);
  if (!cred) return res.status(404).json({ error: 'Verification link not found' });
  if (cred.clientVerified) return res.json({ success: true, message: 'Already verified', credential: cred });
  ...
  logActivity(`Client verified job: ${cred.jobType} — $${cred.amount} by ${cred.client}`, 'blockchain');
  saveData(); broadcastSSE('credential:verified', { id: cred.id, client: cred.client, amount: cred.amount });
  await notifySlack(`✅ Client *${cred.client}* verified a payment — $${cred.amount} for *${cred.jobType}*`);
  res.json({ success: true, message: 'Payment verified!', credential: cred });
}));

// ── Public Reputation Profile ──
app.get('/profile/:handle', (req, res) => {
  if (req.headers.accept && req.headers.accept.includes('application/json')) {
    if (req.params.handle.toLowerCase() !== PROFILE_HANDLE.toLowerCase()) return res.status(404).json({ error: 'Profile not found' });
    const verified = db.reputation.filter(r => r.clientVerified); const totalEarnings = verified.reduce((s,r)=>s+Number(r.amount||0),0); const score = Math.min(1000, db.reputation.length*180 + verified.length*40);
    return res.json({ handle: PROFILE_HANDLE, score, level: score>=700?'Elite':score>=400?'Established':'Emerging', totalJobs: db.reputation.length, verifiedJobs: verified.length, totalEarnings, winRate: (() => { const d=db.proposals.filter(p=>['won','lost'].includes(p.status)).length; return d?Math.round(db.proposals.filter(p=>p.status==='won').length/d*100):0; })(), credentials: verified.map(r=>({ jobType:r.jobType, amount:r.amount, date:r.date, paymentRail:r.paymentRail, minted:r.minted, txHash:r.txHash })), lastUpdated: new Date().toISOString() });
  }
  if (req.params.handle.toLowerCase() !== PROFILE_HANDLE.toLowerCase()) return res.status(404).send('<h1>Profile not found</h1>');
  const verified = db.reputation.filter(r => r.clientVerified);
  const totalEarnings = verified.reduce((s,r)=>s+Number(r.amount||0),0);
  ...
  res.setHeader('Content-Type', 'text/html; charset=utf-8');
  res.send(`<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${PROFILE_HANDLE} — HermesWork Profile</title><meta name="description" content="Verified freelance reputation for ${PROFILE_HANDLE}. ${verified.length} client-verified jobs, $${totalEarnings.toLocaleString()} confirmed earnings."><meta property="og:title" content="${PROFILE_HANDLE} on HermesWork"><meta property="og:description" content="${verified.length} verified jobs · $${totalEarnings.toLocaleString()} earnings · Score ${score}/1000"><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:system-ui,sans-serif;background:#0f0f17;color:#e2e8f0;min-height:100vh}header{background:linear-gradient(135deg,#1e1b4b 0%,#312e81 100%);padding:60px 20px;text-align:center}.logo{font-size:14px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#a5b4fc;margin-bottom:32px}.avatar{width:80px;height:80px;background:linear-gradient(135deg,#5046e4,#818cf8);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:32px;font-weight:800;margin:0 auto 16px;border:3px solid rgba(255,255,255,.2)}.handle{font-size:32px;font-weight:800;margin-bottom:4px}.level-badge{display:inline-block;background:rgba(255,215,0,.15);color:gold;border:1px solid rgba(255,215,0,.3);padding:4px 16px;border-radius:20px;font-size:12px;font-weight:700;margin-bottom:24px}.stats{display:flex;justify-content:center;gap:40px;flex-wrap:wrap;margin-top:24px}.stat{text-align:center}.stat-value{font-size:28px;font-weight:800;color:#a5b4fc}.stat-label{font-size:12px;color:#94a3b8;margin-top:4px}.container{max-width:760px;margin:0 auto;padding:48px 20px}h2{font-size:20px;font-weight:700;margin-bottom:20px;color:#f1f5f9}.card{background:#1e1e2e;border:1px solid #2d2d44;border-radius:14px;padding:24px;margin-bottom:16px;display:flex;justify-content:space-between;align-items:flex-start}.card-left h3{font-size:16px;font-weight:700;margin-bottom:6px;color:#f1f5f9}.card-meta{font-size:12px;color:#64748b;margin-top:4px}.card-right{text-align:right}.card-amount{font-size:22px;font-weight:800;color:#a5b4fc}.verified{display:inline-block;background:#052e16;color:#86efac;padding:3px 10px;border-radius:10px;font-size:11px;font-weight:700;margin-top:6px}.chain{font-size:10px;color:#475569;margin-top:4px;font-family:monospace;cursor:pointer}.empty{text-align:center;padding:48px;color:#475569}footer{text-align:center;padding:32px;font-size:12px;color:#334155;border-top:1px solid #1e293b}</style></head><body><header><div class="logo">HermesWork · Verified Reputation</div><div class="avatar">${PROFILE_HANDLE.slice(0,1).toUpperCase()}</div><div class="handle">${PROFILE_HANDLE}</div><div class="level-badge">${level} Freelancer</div><div class="stats"><div class="stat"><div class="stat-value">${score}</div><div class="stat-label">Reputation Score</div></div><div class="stat"><div class="stat-value">${verified.length}</div><div class="stat-label">Verified Jobs</div></div><div class="stat"><div class="stat-value">$${totalEarnings.toLocaleString()}</div><div class="stat-label">Confirmed Earnings</div></div><div class="stat"><div class="stat-value">${winRate}%</div><div class="stat-label">Win Rate</div></div></div></header><div class="container"><h2>Verified Work Records</h2>${verified.length?verified.map(r=>`<div class="card"><div class="card-left"><h3>${r.jobType}</h3><div class="card-meta">${r.date} · ${r.paymentRail||'Stripe'}</div>${r.txHash?`<div class="chain" onclick="navigator.clipboard.writeText('${r.txHash}')" title="Click to copy tx">${r.txHash.slice(0,16)}…${r.txHash.slice(-6)}</div>`:''}</div><div class="card-right"><div class="card-amount">$${Number(r.amount).toLocaleString()}</div><div class="verified">✓ Client Verified</div>${r.minted?'<div style="font-size:10px;color:#4ade80;margin-top:4px">⛓ On-chain</div>':''}</div></div>`).join(''):`<div class="empty"><div style="font-size:40px;margin-bottom:12px">🛡</div><p>No verified jobs yet.</p></div>`}</div><footer>HermesWork · Payment-Backed Reputation · <a href="${PUBLIC_BASE_URL}" style="color:#5046e4">${PUBLIC_BASE_URL.replace('https://','')}</a></footer></body></html>`);
});
...
app.post('/api/clients', requireApiKey, validate({ name:{required:true,maxLen:100} }), (req, res) => {
  ...
  const existing = db.clients.find(c => String(c.name).toLowerCase() === name.toLowerCase()); if (existing) return res.status(409).json({ error:'Client already exists', client: existing });
...
app.post('/api/proposals', requireApiKey, validate({ title:{required:true,maxLen:200}, client:{required:true,maxLen:100}, status:{enum:['pending','won','lost']} }), (req, res) => {
  ...
  db.proposals.push(proposal); logActivity('Proposal sent: '+proposal.title+' to '+proposal.client,'proposal'); saveData(); broadcastSSE('proposal:created',{id:proposal.id,client:proposal.client}); res.status(201).json({ success:true, proposal });
});
...
app.get('/api/analytics', (req, res) => {
  ...
  const avgLast3=months.slice(3).reduce((s,v)=>s+v,0)/3;
  const pipelineValue=db.proposals.filter(p=>p.status==='pending').reduce((s,p)=>s+Number(p.amount||0),0);
  const forecastNext=Math.round(avgLast3+pipelineValue*(winRate/100));
  res.json({ revenueOverTime:months, winRateTrend:[0,0,0,0,0,winRate], daysToPayment:Array(5).fill(0).concat([avgDays||0]), credentialsPerMonth:creds, monthLabels, months:monthLabels, totalRevenue:months.reduce((s,v)=>s+v,0), winRate, forecastNext, pipelineValue, avgDaysToPayment:avgDays,
    hypotheses:[
      ...
      {metric:'Revenue Forecast (Next Mo)',baseline:0,target:5000,current:forecastNext,unit:'',prefix:'$',hit:forecastNext>=5000}
    ]
  });
});
...
app.post('/webhooks/stripe', asyncWrap(async (req, res) => {
  ...
  if (event.type==='invoice.paid'||event.type==='payment_intent.succeeded') {
    ...
    if (invoice&&invoice.status!=='paid') {
      ...
      saveData(); broadcastSSE('invoice:paid',{id:invoice.id,amount:invoice.amount,client:invoice.client});
      await notifySlack(`💳 Stripe payment confirmed — *${invoice.id}* — $${invoice.amount} from *${invoice.client}*\nVerify: ${PUBLIC_BASE_URL}/verify/${verifyToken}`);
    }
  }
